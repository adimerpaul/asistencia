...
<head>
    ...
    <style>
        ...
        .thead-lite th{ background:#efefef; font-weight:600; text-transform:uppercase; letter-spacing:.2px; }
...
<body>
<div class="title">REPORTE DE NOTAS (SER · SABER · HACER · DECIDIR)</div>
...
<table>
    ...
    <tr>
        ...
        <th class="center">SER (10)</th>
        <th class="center">SABER (35)</th>
        <th class="center">HACER (35)</th>
        <th class="center">DECIDIR (10)</th>
        <th class="center">TOTAL (90)</th>
    </tr>
    </thead>
    <tbody>
    @foreach ($asignacion->estudiantes as $i => $est)
        @php
            ...
            $saber = (int)($n->saber ?? 0);
            $hacer = (int)($n->hacer ?? 0);
            $decidir = (int)($n->decidir ?? 0);
            $total = $ser + $saber + $hacer + $decidir;
        @endphp
        <tr>
            <td class="center">{{ $i+1 }}</td>
            <td class="nowrap">{{ $est->nombre }}</td>
            <td class="center nowrap">{{ $est->ci }}</td>
            <td class="center">{{ $ser }}</td>
            <td class="center">{{ $saber }}</td>
            <td class="center">{{ $hacer }}</td>
            <td class="center">{{ $decidir }}</td>
